...
def get_loader(config, shuffle=True):
    ...
    def collate_fn(batch):
        ...
        for sample in batch:
            ...
            encoded_bert_sent = bert_tokenizer(
                text, max_length=52, add_special_tokens=True, padding='max_length', truncation=True)
    ...
    data_loader = DataLoader(
        ...
        generator=getattr(config, '_generator', None),
        worker_init_fn=getattr(config, '_worker_init_fn', None),
    )

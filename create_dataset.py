...
class MOSI:
    def __init__(self, config):
        ...
        # sdk_dir is not needed when mmsdk is pip-installed
        if config.sdk_dir is not None:
        ...
        try:
            ...
            # mmsdk saves the FACET 4.1 file as CMU_MOSI_Visual_Facet_41.csd (47-dim features)
            ...
            # Save glove embeddings cache (optional; model uses BERT for text, so GloVe is not required)
            if getattr(config, 'word_emb_path', None) and os.path.exists(config.word_emb_path):
                self.pretrained_emb = pretrained_emb = load_emb(word2id, config.word_emb_path)
            else:
                self.pretrained_emb = pretrained_emb = None
